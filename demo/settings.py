...
TIME_ZONE = None
gettext = lambda s: s
direxist = os.path.isdir("/tmp/ison/")
if not direxist:
    os.system("mkdir /tmp/ison/")
...
ALLOWED_INCLUDE_ROOTS =(os.path.join(os.path.dirname(__file__), 'static/img'),os.path.join(os.path.dirname(__file__), 'static/img'))
...
FILE_URL = os.path.join(os.path.dirname(__file__), '/tmp/ison/')
...
GRAPPELLI_ADMIN_TITLE = '<a href="/" title="ISON">ISON</a>'
...
DEFAULT_FROM_EMAIL = 'webmaster@ison.metastudio.org'
LOGIN_REDIRECT_URL = '/'
EMAIL_SUBJECT_PREFIX='[ison-error-reporting]'
